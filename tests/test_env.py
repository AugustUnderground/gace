...
def _test_env(env: gym.Env):
    assert isinstance(env, gym.Env), \
           'The env must inherit from gym.Env.'
    ...
    assert isinstance(env.observation_space, gym.spaces.Space), \
           'The observation_space must inherit from gym.spaces.Space.'
    assert isinstance(env.action_space, gym.spaces.Space), \
           'The action_space must inherit from gym.spaces.Space.'

    if isinstance(env.action_space, gym.spaces.Dict):
        for s in env.action_space.values():
            assert isinstance(s, gym.spaces.Space), \
                  'All spaces within a dict action space must inherit from gym.spaces.Space'

    assert len(env.observation_space.shape) in [1, 3], \
           'The shape of the observation space should be ∈ [1,3].'
    ...
    assert env.observation_space.shape[0] == obs.shape[0], \
           'The dimensions of the observation space do not match.'
    ...
    assert env.action_space.shape[0] == act.shape[0], \
           'The dimensions of the action space do not match.'

    res = env.step(act)

    assert len(res) == 4, \
           'The `step` function must return a tuple with four elements.' 
    ...
    assert isinstance(r, (float, int)), \
           'The reward returned by `step` must be of type float.'

    assert isinstance(d, bool), \
           'The done signal returned by `step` must be of type bool.'

    assert isinstance(i, dict), \
           'The info returned by `step` must be of type dict'

    modes = env.metadata.get('render.modes')

    assert modes is not None, \
           'The env does not specify any render mode.'

    assert 'human' in modes, \
           'The env does not specify a human render mode.'
...
def test_xh035_nand_v1():
    _test_inv_v1('gace:nand4-xh035-v1')

def test_sky130_nand_v1():
    _test_inv_v1('gace:nand4-sky130-v1')

def test_gpdk180_nand_v1():
    _test_inv_v1('gace:nand4-gpdk180-v1')

def test_xh035_st_v1():
    _test_inv_v1('gace:st1-xh035-v1')

def test_sky130_st_v1():
    _test_inv_v1('gace:st1-sky130-v1')

def test_gpdk180_st_v1():
    _test_inv_v1('gace:st1-gpdk180-v1')
