...
(defclass OP5Env [SingleEndedOpAmpEnv]
  """
  Derived amplifier class, implementing the Unsymmetrical Cascode Amplifier.
  ...
  (defn __init__ [self &optional ^str [pdk-path None] ^str [ckt-path None] 
    ...
    ;; ACE Environment ID
    (setv self.ace-env "op5")

    ;; Initialize parent Environment.
    (.__init__ (super OP5Env self) 
               [pdk-path] ckt-path
               nmos-path pmos-path
               max-moves
...
(defclass OP5XH035Env [OP5Env]
  """
  Unsymmetrical Cascode Amplifier in XH035 Technology.
  """
  ...
  (defn __init__ [self &optional ^str [pdk-path None] ^str [ckt-path None] 
    ...
    (.__init__ (super OP5XH035Env self) :pdk-path pdk-path :ckt-path ckt-path
